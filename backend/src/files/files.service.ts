		...
		try {
			...
			try {
				await db
					...
					.where(eq(rolePermissionsTable.roleId, fileMgtRole.id));
				await db
					.delete(userRolesTable)
					.where(eq(userRolesTable.roleId, fileMgtRole.id));
				await db
					.delete(permissionsTable)
					.where(eq(permissionsTable.id, fileRwPermission.id));
				...
				try {
					...
					if (file) {
						const roles = await db
							.select()
							.from(rolesTable)
							.where(eq(rolesTable.name, `${user.username} rw file-mgt ${id}`));
						for (const role of roles) {
							...
							await db
								.delete(permissionsTable)
								.where(eq(permissionsTable.resourceId, id));
					...
					console.error(
						`Error cleaning up database record for file ${id}:`,
						cleanupError,
					);
				}
			}

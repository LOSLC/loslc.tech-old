  ...
  return (
    <div className="space-y-2">
      ...
      <div className="border rounded-md">
        <Command>
          <div className="flex items-center px-2" data-slot="command-input-wrapper">
            <CommandInput
              value={query}
                ...
                <CommandEmpty>
                  <div className="px-2 py-3 text-sm text-muted-foreground">
                    ...
                    {canCreate && !hasOptions && (
                      ...
                      >
                        <Plus className="w-4 h-4" /> {t('forum.tags.create', { name: trimmed })}
                      </button>
                    )}
                  </div>
                </CommandEmpty>
                ...
                {hasOptions && canCreate && (
                  <CommandGroup>
                    <CommandItem onSelect={() => createIfMissing(trimmed)}>
                      <Plus className="w-4 h-4 mr-2" /> {t('forum.tags.create', { name: trimmed })}
                    </CommandItem>
                  </CommandGroup>
                )}
              </>

...
import { forumApi, forumKeys, type GetForumPostsQueryDTO, type CreateForumCommentDTO, type VoteDTO } from "@/lib/api/forum";
...
export function useAddComment(postId: string) {
  ...
  return useMutation({
    mutationFn: (data: CreateForumCommentDTO) => forumApi.addComment(postId, data),
    onSuccess: () => {
...
export function useVoteComment(postId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ commentId, voteType }: { commentId: string; voteType: 1 | -1 }) =>

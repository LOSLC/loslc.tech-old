...
export const blogKeys = {
	...
	likesCount: (id: string) =>
		[...blogKeys.all, "post", id, "likes", "count"] as const,
...
export function useInfiniteBlogPosts(
	baseQuery: Omit<GetBlogPostsQueryDTO, "offset"> = {},
) {
...
export function useSearchBlogPosts() {
	...
	return useMutation({
		mutationFn: (searchQuery: GetBlogPostsQueryDTO) =>
			blogApi.getBlogPosts(searchQuery),
...
export function useCachedBlogPostsCount(query: GetBlogPostsQueryDTO = {}) {
	const queryClient = useQueryClient();
	const cachedData = queryClient.getQueryData<BlogPostDTO[]>(
		blogKeys.search(query),
	);
...
export function useLikeBlogPost() {
	...
	return useMutation({
		...
		onMutate: async (postId) => {
			...
			const previousPost = queryClient.getQueryData<BlogPostDTO>(
				blogKeys.post(postId),
			);
			const previousLiked = queryClient.getQueryData<boolean>(
				blogKeys.liked(postId),
			);
			const previousCount = queryClient.getQueryData<number>(
				blogKeys.likesCount(postId),
			);

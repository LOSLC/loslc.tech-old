...
export const adminApi = {
	...
	dashboard: {
		getPopularBlogPosts: async (limit = 5): Promise<BlogPostDTO[]> => {
			...
			const promise = api
				.get("blog/posts/popular", { searchParams })
				.json<BlogPostDTO[]>();
	...
	accessManagement: {
		...
		createRole: async (data: CreateRoleDTO): Promise<RoleDTO> => {
			const promise = api
				.post("accessmgt/roles", { json: data })
				.json<RoleDTO>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		getPermissionById: async (permissionId: string): Promise<PermissionDTO> => {
			const promise = api
				.get(`accessmgt/permissions/${permissionId}`)
				.json<PermissionDTO>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		createPermission: async (
			...
			const promise = api
				.post("accessmgt/permissions", { json: data })
				.json<PermissionDTO>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		deletePermission: async (
			...
			const promise = api
				.delete(`accessmgt/permissions/${permissionId}`)
				.json<{ message: string }>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		getUserRoles: async (userId: string): Promise<RoleDTO[]> => {
			const promise = api
				.get(`accessmgt/users/${userId}/roles`)
				.json<RoleDTO[]>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		assignRoleToUser: async (
			...
			const promise = api
				.post("accessmgt/users/roles/assign", { json: data })
				.json<{ message: string }>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		revokeRoleFromUser: async (
			...
			const promise = api
				.delete("accessmgt/users/roles/revoke", { json: data })
				.json<{ message: string }>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		getRolePermissions: async (roleId: string): Promise<PermissionDTO[]> => {
			const promise = api
				.get(`accessmgt/roles/${roleId}/permissions`)
				.json<PermissionDTO[]>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		assignPermissionToRole: async (
			data: PermissionAssignmentDTO,
		): Promise<{ message: string }> => {
			const promise = api
				.post("accessmgt/roles/permissions/assign", { json: data })
				.json<{ message: string }>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
		...
		revokePermissionFromRole: async (
			...
			const promise = api
				.delete("accessmgt/roles/permissions/revoke", { json: data })
				.json<{ message: string }>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);
	...
	blog: {
		...
		getAllPosts: async (
			...
			const promise = api
				.get(`blog/admin/posts?${searchParams.toString()}`)
				.json<BlogPostDTO[]>();
			const [response, error] = await resolveRequest(promise);
			if (error) {
				throw new Error(error.message);

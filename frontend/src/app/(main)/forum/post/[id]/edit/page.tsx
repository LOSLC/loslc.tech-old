...
export default function EditForumPostPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: UpdateForumPostDTO) => forumApi.updatePost(id, data),
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold">{t('forum.editPage.title', 'Edit post')}</h1>
          <Link href={`/forum/post/${id}`} className="text-sm text-muted-foreground hover:underline">{t('forum.editPage.back', 'Back to post')}</Link>
        </div>
        {isLoading ? (
          ...
          <form onSubmit={submit} className="space-y-6">
            ...
            <div className="flex gap-3">
              <Button type="submit" disabled={updateMutation.isPending}>
                {updateMutation.isPending ? t('forum.editPage.actions.saving', 'Saving...') : t('forum.editPage.actions.save', 'Save changes')}
              </Button>
